...
namespace Neudesic.AzureStorageExplorer.ViewModel
{
    ...
    public class BlobViewModel : ViewModelBase
    {
        ...
        public ObservableCollection<Property> Properties
        {
            get
            {
                ...
                if (properties == null)
                {
                    ...
                    if (!editable) properties.Add(new Property("AbsoluteUri", blob.Uri.AbsoluteUri));
                    ...
                    if (!editable) properties.Add(new Property("LastModifiedUtc", blob.Properties.LastModified.ToString()));
        ...
        public BlobViewModel(BlobDescriptor blob)
        {
            ...
            {
                foreach(string key in blob.CloudBlob.Metadata.Keys)
                {
                ...
                switch (prop.PropertyName)
                {
                    case "(Name)":
                        if (prop.PropertyValue != BlobDescriptor.BlobName(blob))
                        {
                            ...
                            if (container != null)
                            {
                                CloudBlockBlob newBlob = container.GetBlockBlobReference(prop.PropertyValue); Encoding encoding = Encoding.UTF8;
